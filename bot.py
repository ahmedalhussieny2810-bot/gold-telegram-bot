...
# VERSION
# =========================================================
VERSION = "ALHUSSIENY_FACEBOOK_PUBLIC_FIX_2026_08_12_V10"
...
async def facebook(text):
    ...
    print("\n" + "=" * 60, flush=True)
    print("FACEBOOK PUBLIC PUBLISH V10 START", flush=True)
    ...
    # Build a canonical public URL from verified Page ID.
    public_page_id = str(FACEBOOK_PAGE_ID)
    if isinstance(post.get("from"), dict) and post["from"].get("id"):
        public_page_id = str(post["from"]["id"])

    post_part = str(post_id).split("_", 1)[-1]
    permalink = (
        f"https://www.facebook.com/{public_page_id}/posts/{post_part}"
    )
    ...
    public_ok = (
        ...
        and (
            not source_id
            or str(source_id) == str(FACEBOOK_PAGE_ID)
        )
    )

    if not public_ok:
        diagnostic = [
            "❌ Facebook: البوست اتعمل لكن مش Public بالشكل المطلوب.",
            "",
            f"🆔 Post ID: {post_id}",
            ...
            "",
            "⚠️ البوت لن يعتبر العملية ناجحة.",
        ]
        ...
        diagnostic += [
            "",
            "لو أنت شايف البوست من حساب الأدمن فقط، "
            "فده يؤكد إن Meta أنشأت Object لكن حالته ليست Public "
            "أو أن إعدادات الصفحة/النشر تمنع ظهوره للزوار."
        ]
    ...
    result_lines = [
        "✅ Facebook: تم النشر Public بنجاح.",
        "",
        f"📘 الصفحة: {page_name}",
        f"🆔 Page ID: {FACEBOOK_PAGE_ID}",
        f"🆔 Post ID: {post_id}",
    ...
    if post.get("created_time"):
        result_lines += [
            "",
            f"🕒 وقت الإنشاء: {post['created_time']}",
        ]

    result = "\n".join(result_lines)

    print("FACEBOOK FINAL PUBLIC SUCCESS", flush=True)
...
async def buttons(update, context):
    ...
    if c in ("pub_both", "pub_tg", "pub_fb"):
        ...
        if fb_ok:
            result_lines.append("✅ Facebook: تم النشر Public.")
