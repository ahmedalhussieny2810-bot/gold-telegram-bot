    ...
    try:
        with connection.cursor() as cursor:
            ...
            add_column_if_missing(
                cursor, "Categories", "parent_id",
                "BIGINT UNSIGNED NULL"
            )
            add_column_if_missing(
                cursor, "Categories", "name",
                "VARCHAR(255) NOT NULL DEFAULT ''"
            )
            add_column_if_missing(
                cursor, "Categories", "created_at",
            ...
            add_column_if_missing(
                cursor, "Products", "category",
                "VARCHAR(255) NULL"
            )
            add_column_if_missing(
                cursor, "Products", "Photo_id",
                "TEXT NOT NULL"
            )
            add_column_if_missing(
                cursor, "Products", "category_id",
                "BIGINT UNSIGNED NULL"
            )
            add_column_if_missing(
                cursor, "Products", "name",
                "VARCHAR(255) NULL"
            )
            add_column_if_missing(
                cursor, "Products", "description",
                "TEXT NULL"
            )

            # Kept only for compatibility with old database data.
            # The new bot does NOT ask for or display them.
            add_column_if_missing(
                cursor, "Products", "code",
                "VARCHAR(100) NULL"
            )
            add_column_if_missing(
                cursor, "Products", "price",
                "DECIMAL(15,2) NULL"
            )

            # Migrate old products that only have the old text category.
            cursor.execute(
                """
...
def add_product(cid, photo, name, description):
    connection = db()
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                ...
                (category, Photo_id, category_id, name, description)
                VALUES(%s, %s, %s, %s, %s)
                """,
                (
                    category_name,
...
def products(cid):
    return many(
        """
        SELECT id, Photo_id, name, description
        FROM Products
        WHERE category_id = %s
        ORDER BY id DESC
        """,
...
def comparison(price):
    ...
    if difference > 0:
        return f"📈 عيار 21 ارتفع {difference} جنيه عن أول سعر أمس"

    if difference < 0:
        return f"📉 عيار 21 انخفض {abs(difference)} جنيه عن أول سعر أمس"
...
def product_keyboard(sub_id, index):
    product_list = products(sub_id)
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "📝 وصف المنتج",
                callback_data=f"pd:{sub_id}:{index}",
            )
        ]
    ]

    # This opens WhatsApp Business with a ready-made inquiry.
    # It does NOT attach the Telegram photo, because WhatsApp
    # deep links cannot attach an image automatically.
    name = product["name"] or "المنتج"

    message = (
        "السلام عليكم، أريد الاستعلام عن هذا المنتج 💍\n"
        f"اسم المنتج: {name}"
    )
    ...
    if index > 0:
        navigation.append(
            InlineKeyboardButton(
                "⬅️ السابق",
                callback_data=f"pv:{sub_id}:{index - 1}",
            )
        )
    ...
    if category:
        keyboard.append([
            InlineKeyboardButton(
                "↩️ رجوع للقسم",
                callback_data=f"cm:{category['parent_id']}",
            )
        ])
...
def product_caption(product, show_description=False):
    name = product["name"] or "منتج"

    if show_description:
        description = (
            product["description"]
            or "لا يوجد وصف لهذا المنتج."
        )
        return f"💍 {name}\n\n📝 {description}"
...
async def text(update, context):
    ...
    if state == "sub":
        ...
        result = add_sub(parent, t)

        await update.message.reply_text(
            "⚠️ القسم الفرعي موجود بالفعل."
            if result is None
            else f"✅ تم إضافة القسم الفرعي.\n\n🟡 {t}",
            reply_markup=cat_menu(),
        )
...
async def buttons(update, context):
    ...
    if choice.startswith("cm:"):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    f"🟡 {sub['name']} ({sub['product_count']})",
                    callback_data=f"cs:{sub['id']}",
                )
            ]
    ...
    # =====================================================
    # OPEN SUBCATEGORY PRODUCTS
    # =====================================================

    if choice.startswith("cs:"):
        try:
            sub_id = int(choice.split(":", 1)[1])
        except ValueError:
            return
            ...
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [
                        InlineKeyboardButton(
                            "⬅️ رجوع",
                            callback_data=f"cm:{sub['parent_id']}",
                        )
                    ]
                ]),
            )
        ...
        # Product #1 is sent as a photo message.
        product = product_list[0]

        await query.edit_message_text(
            f"🟡 {sub['name']}\n\n"
            f"عدد المنتجات: {len(product_list)}\n\n"
            "👇 أول منتج:"
        )
        ...
        await query.message.reply_photo(
            ...
            reply_markup=product_keyboard(sub_id, 0),
        )
        return
    ...
    if choice.startswith("pv:"):
        try:
            _, sub_id_text, index_text = choice.split(":")
            sub_id = int(sub_id_text)
        ...
        try:
            await query.edit_message_media(
                ...
                reply_markup=product_keyboard(sub_id, index),
            )
        except Exception as e:
            print("Product Navigation Error:", e)
    ...
    if choice == "viewcats":
        ...
        for main in cats():
            lines.append("💍 " + main["name"])

            for sub in cats(main["id"]):
                lines.append(
                    f"   └ 🟡 {sub['name']} "
    ...
    if choice == "rename":
        ...
        for main in cats():
            ...
            for sub in cats(main["id"]):
                keyboard.append([
                    InlineKeyboardButton(
                        f"✏️ {main['name']} → {sub['name']}",
                        callback_data=f"rp:{sub['id']}",
                    )
                ])
    ...
    if choice == "deletecat":
        ...
        for main in cats():
            ...
            for sub in cats(main["id"]):
                keyboard.append([
                    InlineKeyboardButton(
                        f"🗑 {main['name']} → {sub['name']}",
                        callback_data=f"dc:{sub['id']}",
                    )
                ])
    ...
    if choice.startswith("pm:"):
        ...
        if not sub_categories:
            await query.edit_message_text(
                "❌ القسم الرئيسي لا يحتوي أقسام فرعية.\n\n"
    ...
    if choice == "viewprod":
        ...
        if len(product_list) > 50:
            lines.append(
                f"عرض أول 50 من أصل {len(product_list)} منتج."
            )
    ...
    if choice.startswith("dp:"):
        if not is_admin(update):
            return

        try:
            product_id = int(choice.split(":", 1)[1])
        except ValueError:
            return

        await query.edit_message_text(
            f"⚠️ تأكيد حذف المنتج #{product_id}",
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton(
                        "✅ احذف",
                        callback_data=f"cdp:{product_id}",
                    )
                ],
                [
                    InlineKeyboardButton(
    ...
    if choice in ("pub_both", "pub_tg", "pub_fb"):
        if not is_admin(update):
            return

        text_to_publish = context.user_data.get("price_text")
        price = context.user_data.get("price")

        if not text_to_publish or price is None:
            await query.edit_message_text(
                "❌ السعر انتهى.",
        ...
        if choice in ("pub_both", "pub_tg"):
            tg_ok = await tg(
                context,
                text_to_publish,
            )

        if choice in ("pub_both", "pub_fb"):
            fb_ok = await facebook(text_to_publish)
